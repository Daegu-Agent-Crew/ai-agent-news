...
7월 연구 동향은 평가에서 최적화로의 전환을 보여준다. 벤치마크가 '얼마나 잘하는가'를 측정한다면, AutoTool과 SkillWeaver는 '어떻게 더 저렴하게 잘할 수 있는가'를 답한다. 두 연구 모두 도구 사용 패턴의 통계적 구조를 활용하며, 그래프 기반 오케스트레이션(LangGraph, ADK 2.0)과 자연스럽게 연결된다.

SkillWeaver의 99% 토큰 절감은 단순한 비용 절감을 넘어, 수백 개의 MCP 도구를 운영하는 기업 환경에서 에이전트가 경제적으로 작동할 수 있게 하는 인프라적 돌파구다. 이는 산업 통계가 보여주는 79% 채택/11% 생산 격차를 좁히는 직접적 해결책이 될 수 있다.
