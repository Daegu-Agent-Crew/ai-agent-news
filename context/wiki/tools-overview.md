...
- **카테고리**: tools
- **관련 뉴스 수**: 54
- **최종 업데이트**: 2026-09-07 (23차 갱신)

## 요약
에이전트 도구 생태계가 빠르게 분화하고 있다. 브라우저 자동화, MCP 서버, 터미널 작업 등 각 영역별 전문 도구가 등장하면서, 에이전트 개발 스택이 성숙 단계에 진입했다. MCP(Model Context Protocol)가 200+ 서버 구현체를 확보하며 사실상 표준으로 자리잡았고, 도구 간 상호운용성이 빠르게 표준화되고 있다. 8월에는 코딩 도구가 에디터를 넘어 코드 호스팅·소프트웨어 팩토리·추론 배포까지 개발 파이프라인 전 계층의 인프라로 확장하고 있다. 동시에 교육·에이전트 평가·가정 운영 등 개발 스택 밖 영역으로 AI 도구가 확산 중이다. 8월 말에는 도구 스택 전체를 하드웨어에 싣는 로컬-first 플랫폼(Perplexity Portable Computer)과 물리 세계 무인 운영(Gatik)으로 실행·적용 경계가 확장되는 동시에, 기능 분화가 만드는 브랜딩 파편화(Gemini)로 경험 경계의 역행도 함께 나타났다. 9월 첫째 주에는 생태계 주변부 3개 계층이 동시에 상업화됐다 — 스킬·MCP 서버를 지속 검증하는 에이전트 공급망 보안(AIR), 행동 묘사만으로 학습 데이터셋을 합성하는 생성 API(Adaption Labs), 소비자 개인 미디어 라이브러리를 직접 운영하는 개인 에이전트(Gemini Spark)가 그것이다. 이어 저작권 소송 압력이 도구의 정책 계층에 도달했다 — Claude 시스템 프롬프트가 가사·캐릭터 방어 조항으로 재편되며, 프롬프트가 곧 정책 엔진으로 기능하는 첫 실측 사례가 공개됐다.
